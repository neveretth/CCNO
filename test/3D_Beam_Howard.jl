...
function primary_test!(x_store, y_store, z_store)
    ...
    # y line config
    # x = [fill(0.5, half_zone_number); fill(0.5, half_zone_number)]
    # y = CCNO.generate_x_array(N_sites_eachflavor, L)
    # z = [fill(0.5, half_zone_number); fill(0.5, half_zone_number)]
    # p = hcat(CCNO.generate_py_array(params.N_sites), CCNO.generate_px_array(params.N_sites, Enu_e, Enu_ē), CCNO.generate_pz_array(params.N_sites))
    ...
    # Unused
    energy_sign = zeros(zone_number)
    # for i = 1:zone_number
    #     energy_sign[i] = (p[i, 1] + p[i, 2] + p[i, 3]) > 0 ? 1 : -1
    # end
    ...
    # Psi = productMPS(s, n -> (n % half_zone_number) <= qtr_zone_number ? "Up" : "Dn")
    # spin_vec = ["Up", "Up", "Up", "Up", "Dn", "Dn", "Dn", "Dn", "Up", "Up", "Up", "Up", "Dn", "Dn", "Dn", "Dn"]
    ...
    CCNO.evolve(params, state, x_store, y_store, z_store, zone_number)
    ...
    # Tell me why in the name of all that is holy it would ever be appropriate to have this as an assertion?????
    if abs((Im_Ω - analytic_growth_rate) / analytic_growth_rate) < tolerance
        print("Analytic test passed!")
    else
        print("Analytic test FAILED.")
    end
...
macro run()
    return primary_test!(x_store, y_store, z_store)
end
